...
/**
 * @author Bela Ban
 * @version $Id: GridStore.java,v 1.1 2010/01/07 08:06:57 belaban Exp $
 */
public class GridStore implements IWebdavStore {
    ...
    private final ReplCache<String,byte[]> data;
    ...
    public GridStore(File root) {
        data=new ReplCache<String,byte[]>(PROPS, "data-cluster");
        metadata=new ReplCache<String, GridFile.Metadata>(PROPS, "metadata-cluster");
        try {
            data.start();
            metadata.start();
        }
        ...
        fs=new GridFilesystem(data, metadata, (short)1, 4000);
